// класс соотвесвтует состоянию подготовки к матчу 
const shipDatas = [
...
class PreparationScene extends Scene {
	...
	update() {
		...
		if (!this.draggedShip && mouse.left && !mouse.pLeft) {
			//определяем находиться ли мышка над кораблем
		...
		// Перетаскивание
		//если зажата лкм и есть корабль
		...
		if (!mouse.left && this.draggedShip) {
			...
			const { left, top } = ship.div.getBoundingClientRect(); // кординаты верхнего левого корабля 
			const { width, height } = player.cells[0][0].getBoundingClientRect(); //ширина и высота ячейки 
			...
			const cell = player.cells
				.flat()
				.find((cell) => isUnderPoint(point, cell)); //находится ли ячейка под точкой

			if (cell) { //если нашли, то размещаем корабль
	...
	startComputer(level) {
		//найдем все свободные клетки 
		const matrix = this.app.player.matrix;
		//клетки без кораблей
		const withoutShipItems = matrix.flat().filter((item) => !item.ship);
		let untouchables = [];
		//ячейки в которые бот стрелять не будет генерируются в getRandomSeveral
		if (level === "simple") {
